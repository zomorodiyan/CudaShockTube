...
void ShockTube::allocDeviceMemory() {
	...
	cudaErrorCheck(cudaMalloc((void **)&d_step, sizeof(double)));
...
void ShockTube::freeDeviceMemory() {
	...
	cudaErrorCheck(cudaFree(d_step));
	...
	*d_h = *d_length / (nbrOfGrids - 1);  // space grid size
	...
	for(int i = index; i < nbrOfGrids; i+= stride){
		...
		if (index < nbrOfGrids){
			...
			d_vol[i] = 1;
			*d_step = 0;
			calcCMax(nbrOfGrids, d_u1, d_u2, d_u3, d_gama, d_cMax); 
...
void ShockTube::copyDeviceToHost(const int nbrOfGrids) {
	...
	cudaErrorCheck(cudaMemcpy(&step, d_step, sizeof(double), cudaMemcpyDeviceToHost));
}
